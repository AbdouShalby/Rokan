        ...
        <div class="row d-flex justify-content-center">
            ...
            <?php foreach ($data as $category) { ?>
                <div class="card p-1 me-3 mb-3" style="width: 20rem;">
                    <div class="card-body">
                        <h5 class="card-title mb-3">Name: <span
                                    class="category-data"><?= $category->Category_Name ?></span></h5>
                        <?php if (strlen($category->Category_Description) > 0) { ?>
                            <p class="card-text">Description: <span
                                        class="category-data"><?= $category->Category_Description ?></span></p>
                        <?php } else { ?>
                        ...
                        <div class="actions text-center mt-5 mb-4">
                            <form class="d-inline" action="<?= URLROOT ?>category_edit" method="POST">
                                ...
                                <button type="submit" class="btn custom-button me-3"><i class="fa-solid fa-pen"></i>
                                    Edit
                                </button>
                            </form>
                            <form class="d-inline" action="<?= URLROOT ?>category_delete" method="POST">
                                <input type="number" name="Category_ID" value="<?= $category->Category_ID ?>" hidden>
                                <button type="submit" class="btn custom-button me-3"><i class="fa-solid fa-trash"></i>
                                    Delete
                                </button>
                            </form>
                        </div>
